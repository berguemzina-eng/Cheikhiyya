...
- **Patrón geométrico**: `ui/theme/IslamicPattern.kt` dibuja por código
  una textura de estrellas de ocho puntas (khatam) entrelazadas —
  se usa de fondo en la cabecera de la portada y en el splash. No
  depende de ninguna imagen, es 100% vectorial y offline.
- **Splash screen**: `ui/screens/SplashScreen.kt` — logo real sobre
  un círculo crema (para que la caligrafía verde resalte), con fondo
  verde y el patrón islámico, dura **exactamente 3 segundos**.
- **Logo real ya integrado** ✅ — se usaron los dos logos que
  enviaste:
  - `res/drawable/logo_circular.png` — el medallón circular
    (caligrafía "الطريقة الشيخية الشاذلية"), usado en el splash, la
    portada, el menú lateral y el icono de la app.
  - `res/drawable/logo_horizontal.png` — el logotipo en línea, usado
    en la cabecera de la portada.
  - Como ambos son caligrafía verde sobre fondo transparente, en
    todos los sitios donde se apoyan sobre el verde oscuro de la app
    se les puso un fondo crema detrás (círculo o tarjeta redondeada),
    igual que se ve en tu propia web.
  - El icono de la app (`ic_launcher_foreground.png`) se generó a
    partir del logo circular, con el margen de seguridad que exige
    Android para iconos adaptativos y el mismo círculo crema de
    fondo.

Si más adelante quieres cambiar cualquiera de los dos logos, solo
tienes que reemplazar esos dos archivos PNG en `res/drawable/`
manteniendo el mismo nombre — el código no necesita tocarse.
