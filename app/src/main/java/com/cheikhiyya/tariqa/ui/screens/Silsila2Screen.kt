                ...
                item {
                    ...
                    Column(
                        ...
                        SILSILA_2_NOTE.forEach {
                            Text(it, color = GoldLight, textAlign = TextAlign.Right, fontSize = 13.sp, lineHeight = 22.sp, modifier = Modifier.fillMaxWidth().padding(bottom = 6.dp))
                        }
                    }
...
private fun Silsila2Row(index: Int, link: SilsilaLink2, isLast: Boolean) {
    Row(modifier = Modifier.fillMaxWidth()) {
        ...
        Column(modifier = Modifier.padding(bottom = 18.dp)) {
            Box(
                modifier = Modifier.background(Color.White, RoundedCornerShape(10.dp)).padding(horizontal = 12.dp, vertical = 10.dp),
            ) {
                Column {
                    Text(link.name, color = TextDark, fontWeight = FontWeight.SemiBold, fontSize = 14.sp, textAlign = TextAlign.Right)
                    link.altTeacher?.let {
                        Spacer(Modifier.height(6.dp))
                        Text(it, color = Primary, fontSize = 12.sp, textAlign = TextAlign.Right, lineHeight = 18.sp)
                    }
                }
            }
        }
